function addSongToDisplay(metadata, songObj) {
  ...
  let artist = metadata.common.artist
    ...
    : 'Unknown artist';
  let album = metadata.common.album ? metadata.common.album : 'Unknown album';
...
function setTheme(themeName = 'light') {
  document.getElementById('themeFile').href = path.join(
    'css',
    themeName,
    'variables.css'
  );
...
nextElement.addEventListener('click', (event) => {
  let currSongIndex = playPauseElement.getAttribute('data-song-index');
  if (!currSongIndex || currSongIndex == songIndex - 1)
    console.log('last/no song');
  else {
    let nextSongIndex = parseInt(currSongIndex) + 1;
    let nextElement = document.querySelector(
      `[data-song-index="${nextSongIndex}"]`
    );
...
loopElement.addEventListener('click', (event) => {
  song.loop ? (song.loop = false) : (song.loop = true);
});
